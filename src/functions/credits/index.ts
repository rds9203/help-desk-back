...
import { authenticateToken, calculateMaxCreditForUser, createAuditLog } from '../../../lib/middleware';

async function getCredits(req: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
  ...
  try {
    ...
    return createResponse(200, credits);
  } catch (error) {
...
async function createCredit(req: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
  const corsResponse = handleCors(req);
  if (corsResponse) {
    return corsResponse;
  }
  ...
  try {
    ...
    if (!loanAmount || !installments) {
      return createResponse(400, { error: 'Monto y número de cuotas son requeridos' });
    }
    ...
    if (Number.isNaN(numericAmount) || numericAmount <= 0) {
      return createResponse(400, { error: 'El monto del crédito debe ser un número mayor a 0' });
    }

    if (Number.isNaN(numericInstallments) || numericInstallments <= 0 || numericInstallments > 36) {
      return createResponse(400, { error: 'La cantidad de cuotas debe ser un número entre 1 y 36 meses' });
    }

    const targetUserId = userId ? parseInt(userId) : currentUser.id;
    if (Number.isNaN(targetUserId)) {
      return createResponse(400, { error: 'El usuario indicado no es válido' });
    }
    ...
    if (isCreatingForAnotherUser && !['admin', 'superadmin'].includes(currentUser.role)) {
      return createResponse(403, { error: 'No tienes permisos para crear créditos a otros usuarios' });
    }
    ...
    const existingExposure = user.credits.reduce((total: number, credit: any) => {
